...
void Cola::buscarSolicitud(persona& alumno) {
    int pos = busquedaLineal(alumno);
    if (pos != -1) {
        std::cout << "\nLa solicitud de " << alumno.getNombre() << " esta en la posicion " << pos + 1 << " de la cola.";
        std::cout << "\nFaltan " << pos << " constancias antes de atender su solicitud." << std::endl;
    } else {
        std::cout << "\nNo se encontro la solicitud de " << alumno.getNombre() << ".";
        std::cout << "\nDebe dar de alta una solicitud." << std::endl;
    }
}
